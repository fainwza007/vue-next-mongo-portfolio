...
export const useDateRangeDiff = (startDate, endDate) => {
  ...
  const diffInMonth = Math.ceil(
    moment(dynamicEndDate).diff(moment(startDate), "months", true)
  );
  const diffInYear = Math.ceil(
    moment(dynamicEndDate).diff(moment(startDate), "years", true)
  );
