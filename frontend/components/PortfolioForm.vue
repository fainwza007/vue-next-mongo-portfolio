...
<template>
    ...
    <form @submit="onSubmit">
        ...
        <div class="border border-primary p-5 rounded-md my-2 space-y-2">
            <ul>
                <li v-for="(field, idx) in fields" :key="field.key" class="flex space-x-2">
                    <div class="flex-1">
                        <BaseInput :name="`images[${idx}]`" :key="idx" :value="field.value" v-model="field.value"
                            placeholder="Input the image url" />
                    </div>
                    <BaseButton variant="danger" size="small" class="mb-1" type="button" @click="remove(idx)">
                        Delete
                    </BaseButton>
                </li>
            </ul>
            <BaseButton type="button" size="small" variant="secondary" class="w-full" @click="push('')">
                Add a new image
            </BaseButton>
        </div>
        ...
        <div class="mt-10 flex justify-end">
            <BaseButton v-if="canDelete" type="button" size="small" variant="danger" @click="$emit('delete')">
